...
func TestPostgresConfiguredPortForInstanceReadsConfig(t *testing.T) {
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	t.Setenv("XDG_STATE_HOME", t.TempDir())

	if err := WritePostgresConfigForInstanceWithPort("16", "analytics", "5416"); err != nil {
		t.Fatal(err)
	}

	got, err := PostgresConfiguredPortForInstance("16", "analytics")
	if err != nil {
		t.Fatal(err)
	}
	if got != "5416" {
		t.Fatalf("port = %q, want 5416", got)
	}
}
